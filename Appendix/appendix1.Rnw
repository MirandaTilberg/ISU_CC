...
% \paragraph{Setup}
The trained model, as well as the labeled and processed images used to train the model, are stored in a directory with the following structure. Each new trained model and updated set of images is stored in its own folder of the form "YYYYMMDD-HHMMSS" in the relevant directory.
...
@
% \paragraph{Training the Model Head}
