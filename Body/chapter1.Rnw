...
For novel image classification tasks, Convolutional Neural Networks (CNNs) have become standard/ubiquitous
\footnote{\svp{The Google Trends interest graph shows a massive increase between mid-2014 and 2018 \url{https://trends.google.com/trends/explore?date=2010-01-01%202019-02-18&q=convolutional%20neural%20network,computer%20vision}}}
\svp{\citep{guRecentAdvancesConvolutional2018}}. CNNs have deep architectures that can be trained to identify complex patterns, but they are structurally similar to the human visual architecture and output binary or probabilistic predictions for given labels that are readily interpretable by humans. \svp{As CNNs make use of labeled training data, the predictions generated are for features which are similar to those identified by humans, providing greater face validity to the model.} Once a \svp{CNN} is trained, it is relatively fast and easy to apply the model to new images and obtain classifications.

\subsection{\svp{Convolutional Neural Networks}}

\subsubsection{General Approach}
Visual classification (i.e., assinging a label to an object based on \svp{visual input}) is a complex task that humans do very well\svp{\citep{mallot2000computational}}. Sight is our dominant sense and a significant part of our brain is dedicated to vision, \svp{which means that the structures used to impart meaning on a visual scene have been optimized through millions of years of evolution.} \svp{As Convolutional Neural Networks are organized to mimic the process of object recognition in the human visual cortex, it will be useful to briefly describe that process. }
...
Recall is the proportion of true labels that are assigned by the model. The converse of recall is precision, which measures the proportion of false positives to true positives\svp{This isn't quite true - they're inversely related, but logically one isn't the converse of the other (https://www.creighton.edu/fileadmin/user/HSL/docs/ref/Searching_-_Recall_Precision.pdf). This is a tricky area and requires us to be *very* careful when picking words. The Wiki ROC curve page is very helpful at figuring out what's what.}. An ideal model has high recall and high precision, which means it accurately identifies labels that should be present but does not assign too many false positives in the process. \svp{Transition directly to ROC curves- plot of True positive rate vs. false positive rate, etc. }
...
\svp{Move confusion matrix down a bit. }
A confusion matrix is a cross-tabulation between the observed and the predicted classes of the data. A confusion matrix can help identify which classes are being correctly predicted and which classes are commonly mixed-up with others. \svp{Relate to previously discussed quantities, as in the table at the bottom of this page: https://en.wikipedia.org/wiki/Confusion_matrix}
