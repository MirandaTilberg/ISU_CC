...
ggplot() +
  geom_line(aes(x = fpr, y = tpr), data = unnest(roc_df, roc_plot) %>% arrange(fpr, tpr), size = 1.25) +
  geom_label(aes(x = 1, y = .07, label = sprintf("AUC: %0.2f", auc)), hjust = 1, vjust = -0.2, data = roc_df) +
  geom_point(aes(x = fpr, y = tpr, color = "Equal Error Rate"), data = unnest(roc_df, eer), size = 4) +
