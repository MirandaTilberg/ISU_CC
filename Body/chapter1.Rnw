...
However, in some situations, it is possible to reduce the classification problem to a series of binary classifications; in these situations, which will be described in more detail in Section~\ref{ch3:model-accuracy}, a modification of the confusion matrix is possible to better visualize model performance.
