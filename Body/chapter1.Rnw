...
<<caterpillar-carrot-nodes, fig.height = 3, fig.width = 3, echo = F>>=
...
Recall, or sensitivity, which is the true positive rate, is the \mt{number} of all true positives divided by the number of positive cases in the data. Specificity, which is the true negative rate, is the \mt{number} of all true negatives divided by the number of negative cases in the data. Precision is the sum of all true positives divided by the number of positive predictions made by the model.
...
\svp{XXX - Add a sample ROC curve from ggplot2 to show as an example visualization} \mt{I'm realizing now you probably wanted me to use mtcars or something and create a trivial example. I'll update this later!}

<<example-roc, echo = F>>=


x <- (1:100)/100
y <- log((1:100)+ 1e-5)/log(100)
df <- data.frame(x,y)
ggplot() +
  geom_line(aes(x = x, y = y), data = df, size = 1.25) +
