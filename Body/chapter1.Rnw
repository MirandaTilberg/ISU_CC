...
<<dense-layer, echo = F, fig.width = 5, fig.height = 4, out.width = ".45\\textwidth", fig.cap = "(Left) A densely-connected network with 6 input nodes, one hidden layer with 6 nodes, and 3 output nodes. (Right) A densely-connected network with 6 input nodes, one hidden layer with 6 nodes, 3 output nodes, and a 50\\% dropout rate.", fig.scap = "Densely connected layers with and without dropout.", fig.show =  'hold'>>=
source(file.path(codedir, "Generate_Model_Images.R"))
...
In a multi-label classification task, the standard loss function used is binary cross-entropy loss. For a single input image $x^{(0)}$ with true labels $\mathbf{y} \in \left\{0,1\right\}^C$ and prediction $\mathbf{p} \in [0,1]^C$, where $C$ is the number of output classes,
...
<<example-conf-mat, echo = F, fig.height = 8, fig.width = 8, fig.scap = "Confusion matrix, showing model performance.", fig.cap = "Confusion matrix, showing the performance of a random forest to predict iris species from sepal length from Fisher's Iris Data. Again, it is clear that prediction is very accurate for species Setosa, but the model is not always able to distinguish between the species Versicolor and Virginica.", out.width = ".60\\textwidth">>=
mod$confusion[,1:3] %>%
  as.data.frame() %>%
...
However, in some situations, it is possible to reduce the classification problem to a series of binary classifications; in these situations, a modification of the confusion matrix is possible to better visualize model performance, as described in Section~\ref{sec:model-accuracy}.
