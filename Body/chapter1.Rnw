...
In a multi-label classification task, the standard loss fucntion is Binary Cross-Entropy Loss. For a single input image $x^{(0)}$ with true labels $\mathbf{y} \in \left{0,1\right}^C$ and prediction $\mathbf{\hat{p}} \in (0,1)^C$, where $C$ is the pre-specified number of output classes,
...
<<example-conf-mat, echo = F, fig.height = 4, fig.width = 4, fig.scap = "Confusion matrix, showing model performance", fig.cap = "Confusion matrix, showing the performance of a random forest to predict iris species from sepal length from Fisher's Iris Data. Again, it is clear that prediction is very accurate for species Setosa, but the model is not always able to distinguish between the species Versicolor and Virginica.">>=
mod$confusion[,1:3] %>%
  as.data.frame() %>%
