...
\mt{These sentences belong somewhere in here, tbd where: Convolutional neural networks are a tool for supervised deep-learning that have become standard in recent years for automatic image classification. CNNs are a form of artificial neural network, which were inspired by biological processes in the brain \citep{ANNasModelsofNeuralInfoProcessing}.}
...
\hskip 1cm
\includegraphics[height=.25\linewidth]{carrots.png}
\caption[\svp{Feature Similarity.}]{The features used to distinguish between two similar categories may be subtle, like the features that would differentiate a golden retriever from a labrador retriever.}\label{fig:retrievers}
\end{figure}
...
\end{figure}
 ...
 \mt{Don't we need to cite these images?}
...
\svp{In some cases, fully connected layers may utilize a pruning mechanism known as a dropout rate, which forces a set percentage of the weights to be zero. This results in the maintenance of only the strongest connections between two layers of the model.} \mt{I'm not sure this is true? My understanding is that it prevents any one node from deciding the prediction for a whole class -- like making sure multiple people in an office can do a job in case one is sick.}
