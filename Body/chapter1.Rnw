...
For novel image classification tasks, Convolutional Neural Networks (CNNs) have become standard/ubiquitous (cite). Once a network is trained, it is relatively fast and easy to apply the model to new images and obtain classifications. CNNs have deep architectures that can be trained to identify complex patterns, but they are structurally similar to the human visual architecture and output binary or probabilistic predictions for given labels that are readily interpretable by humans. For a task like classifying outsole shapes, CNNs are a smart and tractable solution/method.
