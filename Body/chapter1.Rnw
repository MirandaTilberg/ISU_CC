...
However, in some situations, it is possible to reduce the classification problem to a series of binary classifications; in these situations, a modification of the confusion matrix is possible to better visualize model performance, as described in Chapter~\ref{ch3:model-accuracy}.
