...
<<example-roc, echo = F, fig.scap = "ROC curves, showing the performance of a random forest to predict iris species from sepal length from Fisher's Iris Data.", fig.cap = "ROC curves, showing the performance of a random forest to predict iris species from sepal length from Fisher's Iris Data. Note that prediction is very accurate for species Setosa, but is not much better than random chance for species Versicolor.">>=
...
for (i in 1:3){
  ...
  ggtitle(as.character(spec[i]) %>% stringr::str_to_title()) +
