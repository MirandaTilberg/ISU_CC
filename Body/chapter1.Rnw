...
According to \citet{grossVariabilitySignificanceClass2013}, the four generally accepted conclusions that can be made from a footwear examination are identification, elimination, class association, and inconclusive. Typically, the ultimate goal of an examination is identification, which is matching a shoe print to an individual shoe, perhaps owned by a suspect of an the investigation. This is difficult because identification to a specific individual's shoe requires the matching of randomly acquired characterisics, such as damage, and often times that information is not available from the print left at the scene \mt{quality, etc}. Most of the \mt{work being done with shoeprints?} involves class association \mt{cite Leslie Hammer?}, which means identifying a group of shoe types that could have made the print. 
...
\mt{The gist:
When we think about categorizing a shoe, we typically think of brand, size, and a broad type (e.g., boot, tennis shoe, dress shoe). Size is not as easy to work with as we would hope, because size is difficult to measure (not consistent across manufacturers/countries) and different shoe styles with same size inside may be differently sized outside \citep{bodziak_footwear_2000}. Model of shoe is also difficult because so many shoes can look similar but come from different brands, models are constantly being introduced or being brought back. However, using geometric shapes to describe outsole designs provides useful information as class characteristics to differentiate different types of shoes \citep{grossVariabilitySignificanceClass2013} and pattern matching "is of considerable evidential value" \citep{hancockInterpretationShoeprintComparison2012}. }
...
\mt{I'll try to gist your gist :D

\begin{itemize}
\item For image classification, Convolutional Neural Networks have become standard/ubiquitous (since better computing, how to say this with reference?)
\item Fast and easy to apply to new images (idk what you meant by "work at scale")
\item Clear structure, match human visual structure architecture
\item Deep architectures can identify complex patterns
\item Output binary or probabilistic predictions for given labels, so easily interpreted by human
\end{itemize}
}
...
Classification tasks are considered single-class when there is a only one binary decision of whether an item belongs to a single class of interest, and multi-class when there are a larger number of classes that an object may belong to. Multi-label classification is the special case of multi-class classification where categories are not mutually exclusive and, thus, an item may fall into a combination of categories simultaneously. While the true classification is a binary decision, it is common for models to predict these classifications on a scale of probability, thus reporting a value between 0 and 1 reflecting how certainly the item can be attributed to a given class.

Evaluating model accuracy on classification requires addressing both the labels that are assigned and the labels that are not. The good scenarios: a true positive is assigning a correct label to an item, and a true negative is when a label is not assigned to an item that should not have that label. Errors can be made in failing to assign a correct label (false positive) or assigning an incorrect label (false negative).

Recall is the proportion of true labels that are assigned by the model. The converse of recall is precision, which measures the proportion of false positives to true positives. An ideal model has high recall and high precision, which means it accurately identifies labels that should be present but does not assign too many false positives in the process.

A confusion matrix is a cross-tabulation between the observed and the predicted classes of the data. A confusion matrix can help identify which classes are being correctly predicted and which classes are commonly mixed-up with others.
