...
\svp{As a general matrix notation for pooling is difficult to specify intuitively, we instead will define a pooling operator, $p$, which applies \autoref{eqn:element-pooling} element-wise, resulting in an output matrix.}
$$p\left(x, f, w, s\right) :=\text{matrix-wise pooling on matrix} x \text{ with function } f, \text{ window } w, \text{ and stride } s$$

For example, taking 2x2 pieces of a feature map and keeping only the largest of the four values reduces the size of the feature map by a factor of 4, as shown in \autoref{fig:max-pooling}. \svp{\citet{krizhevskyImageNetClassificationDeep2012} suggests pooling layers reduce overfitting on certain datasets and increase translation invariance, but using too large of a pooling window can be destructive and limits the total number of convolutional layers which can be combined in a single network\citep{CS231nConvolutionalNeural}. In some CNNs, pooling layers are removed, producing fully convolutional neural networks; dimension reduction is instead performed by increasing the stride and window size of the convolutional layers \citep{springenbergStrivingSimplicityAll2014}.}
...
x^{(\ell)}_k &= p\left(x^{(\ell-1)}, \max, s, s\right) \text{ for max pooling layers w/ stride and window } s\nonumber\\
& \text{\footnotesize where $(i-1)s + 1 \leq i^\ast \leq is$, $(j-1)s + 1 \leq j^\ast \leq js$}\nonumber\\
