...
For novel image classification tasks, Convolutional Neural Networks (CNNs) have become standard/ubiquitous (cite). Once a network is trained, it is relatively fast and easy to apply the model to new images and obtain classifications. CNNs have deep architectures that can be trained to identify complex patterns, but they are structurally similar to the human visual architecture and output binary or probabilistic predictions for given labels that are readily interpretable by humans. For a task like classifying outsole shapes, CNNs are a useful and tractable solution/method.

\subsection{CNN Theory}

\mt{I will read Goldstein this week and fill in this paragraph.}
Visual classification (i.e., assinging a label to an object based on features that can be seen) is a complex task that humans do very well. Sight is our dominant sense and a significant part of our brain is dedicated to vision --> our visual process is refined and skilled (cite somehow?).
(Describe the human visual/classification process: feature detection, routing to the brain, and label application) \svp{Use Sensation \& Perception (Goldstein) heavily here. You might also use some of the computer vision books to compare/contrast.}
...
\subsubsection{Filters and Convolution}
...
\subsubsection{Densely Connected Layers}
