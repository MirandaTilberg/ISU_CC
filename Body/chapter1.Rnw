...
<<activation-functions, echo = F, fig.scap = "Common nonlinear activation functions used in neural networks.", fig.cap = "Activation functions commonly used in neural networks. ReLU, the Rectified Linear Unit, is one of the most common; it simply truncates the feature map at 0, keeping only the positive values. ELU, the Exponential Linear Unit, is an attempt to make a continuous version of ReLU. The sigmoid activation function is used when outputs between 0 and 1 are desired (for instance, in the output layer to produce class probabilities).", out.width = "\\textwidth", fig.width = 6, fig.height = 2>>=
library(tidyverse)
...
x <- seq(-3, 3, .005)
tibble(x = x, Linear = x, Sigmoid = sigmoid(x), ReLU = relu(x), ELU = elu(x, 1)) %>%
  gather(key = key, value = value, -x) %>%
  mutate(key = factor(key, levels = c("Linear", "ReLU", "ELU", "Sigmoid"))) %>%
...
\svp{XXX Go ahead and add the pictures in here - you have them, may as well use them...}
