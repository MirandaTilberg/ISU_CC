...
<<example-roc, echo = F, fig.scap = "ROC curve plots, showing model performance.", fig.cap = "ROC curves, showing the performance of a random forest to predict iris species from sepal length from Fisher's Iris Data. Note that prediction is quite accurate for species Setosa, but is not much better than random chance for species Versicolor.", fig.width = 8, fig.height = 2.5>>=
