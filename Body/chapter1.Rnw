...
<<dense-layer, echo = F, fig.width = 5, fig.height = 4, out.width = ".45\\textwidth", fig.cap = "(Left) A densely-connected layer with 6 input nodes, one hidden layer with 6 nodes, and 3 output nodes. (Right) A densely-connected layer with 6 input nodes, one hidden layer with 6 nodes, 3 output nodes, and a 50\\% dropout rate.", fig.scap = "Densely connected layers with and without dropout.", fig.show =  'hold'>>=
source(file.path(codedir, "Generate_Model_Images.R"))
