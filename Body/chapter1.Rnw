...
For novel image classification tasks, Convolutional Neural Networks (CNNs) \svp{and related methods are current industry best-practice}. CNNs are a form of artificial neural network \svp{which make use of the image convolution operator used by many low-level feature extraction methods}.
...
x^{(\ell)}_k &= \max\left(x_{s\times s, k}^{(\ell-1)}\right) \text{ for max pooling layers with stride and window } s\nonumber\\
x^{(\ell)}_k &= \sigma^\ell\left(Wx\right) \text{ for densely connected layers}\nonumber
\end{align}
...
\left(\frac{\partial L}{\partial W^\ell}\right) &= \underbrace{\frac{\partial L}{\partial \left(W^\ell \ast x^{\ell - 1}\right)}}_\text{gradient} x^{\ell-1}\\
