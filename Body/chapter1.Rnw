...
\caption[Feature Similarity.]{The features used to distinguish between two similar categories may be subtle, like the features that would differentiate a golden retriever from a labrador retriever.\citep{ILSVRC15}}\label{fig:retrievers}
\end{figure}
