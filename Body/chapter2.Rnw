...
\item Drop-out rate
\item Number of epochs and why \svp{Move to Ch3 - first subsec is model results}
\item Train only dense layers
\item Training image here or in results? (I.e. training and validation loss and accuracy plot) \svp{Move to Ch3 - first subsec is model results}
\end{itemize}
