...
\mt{Ideally, I'll assemble three "other" examples and we can add them to the class examples above. It will even out the 2-column grid and makes more sense to include there. Add it to my to-do list :)}
...
Developed by Oxford's Visual Graphics Group, VGG16 is a CNN with 16 "functional" (i.e., convolutional and densely connected) layers and 5 "structural" max-pooling layers. \mt{XXX In contrast to some popular networks, like ResNet, VGG has a relatively simple structure that provides easier training and interpretability with very little sacrificed accuracy.}
