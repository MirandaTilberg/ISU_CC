...
\caption[Classification rule weaknesses]{A shoe which has elements which are not easily classified; the lines in the design do not meet the specified definition of lines (which must be roughly parallel). The rectangle in the center of the shoe can be correctly classified.}\label{fig:classification-is-hard}
\end{figure}
\FloatBarrier
