...
<<b1, fig.width = 11, fig.height = 4, eval = F>>=
library(magick)
b1_imgs <- list.files(here::here("Images", "chapter2", "vgg16layers", "b1"), full.names = T)
c1 <- image_read(b1_imgs[1:4]) %>% image_append()
c2 <- image_read(b1_imgs[5:8]) %>% image_append()
b1 <- image_append(c(c1, c2), stack = T)
b1
image_display(b1)
@
