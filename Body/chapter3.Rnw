...
This modified confusion matrix preserves the ability to identify patterns in over- and under-predictions; for instance, the horizontal band in \autoref{fig:confusion_matrix} indicates that quadrilaterals are overrepresented, as they are predicted more often than they should be for every true label. Similarly, polygons, stars, and triangles are often are underpredicted relative to the images which actually contain these categories since they produce a large number of false positive predictions into other categories, as evidenced by the vertical bands.

\autoref{fig:confusion_matrix} shows the general weaknesses of the model, but does not necessarily suggest where to look as to \emph{why} the model is not performing appropriately. In \autoref{ch3:model-consistency} and \autoref{ch3:heatmap}, alternative visualizations are used to better understand the weaknesses of the model when applied to actual images used to train, test, and validate the model.


\section{Model Consistency}\label{ch3:model-consistency}
In order to examine the model's consistency, and whether it can successfully detect features across a variety of images, several case studies are explored in this section, probing the model's strengths and weaknesses.

\subsection{General shape recognition}

CoNNOR generally performs well when predicting well-defined shapes. \autoref{fig:generic-chevron}, \autoref{fig:generic-text-line}, and \autoref{fig:generic-triangle} show a number of test images containing chevrons, lines, text, and triangles. The chevrons in \autoref{fig:generic-chevron} are usually detected when they are in a regularly repeating pattern, and also when the shape is distinctly pointed; there is one case shown where the chevron base is quite curved and model prediction is low across all classes. Another image shows chevrons that are made up of quadrilaterals; this case is strongly predicted as quadrilateral and only weakly as chevron. Instances like this occur in many contexts throughout the data and have played a significant role in modifying the labeling scheme to consider smaller shapes that are part of larger classes, such as labeling an "o" in text as a circle. 

The images in \autoref{fig:generic-text-line} are primarily all identified as containing text. The two exceptions are both images with low contrast; the darkest image is only identified as containing lines, and the lightest image is predicted with moderate probabilities into nearly every class. Contrast is also an issue in \autored{fig:generic-triangle}, where a white image is correctly labeled as containing text but the triangle is not identified. The darkest images with triangles are correctly identified with moderate probabilities, but the model also assigns similar probabilities to the class of quadrilateral, which is incorrect for these images. These images suggest that CoNNOR may have a general difficulty predicting images with low contrast, producing both false positive and false negative predictions. 
...
In the more homogeneous contrast situation shown in \autoref{fig:chevron}, the color variation does not produce large variations in the output probabilities---the lightest image has only slightly lower probability than the darker images. On the other hand, \autoref{fig:uggs}, \autoref{fig:uggs-with-circle}, \autoref{fig:nb_logo}, and \autoref{fig:nike} show cases where CoNNOR is able to identify shapes when contrast is moderate, but has greater difficulty when contrast is very low. The effect of contrast adjustment on CoNNOR's predictions is further explored in \autoref{ch3:contrast-adjustment}.
...
\subsection{Contrast Adjustment}\label{ch3:contrast-adjustment}
...
A heatmap is a visual diagnostic tool that displays the portions of an image that are most significant to classification for a given class. Belonging to a category of techniques called class activation map (CAM) visualizations, heatmaps are computed by taking the output feature map of a convolutional layer---in this case, the final convolutional layer of VGG16---and weighting each channel of the feature map by the gradient of the class with respect to the channel \citep{chollet_allaire_2018}. Heatmaps provide useful information regarding which locations in an image contribute most to CoNNOR's image predictions.
...
<<chevron-circle-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.scap = "Heatmaps of an image containing chevrons and circles.", fig.cap = "Heatmaps of an image containing chevrons and circles. Chevrons are identified by sharp corners and parallel lines, but the thick ribbon between the two shape sections is falsely identified as a quadrilateral.", fig.pos = 'h'>>=
heatmaps <- list.files(imgdir, pattern = "^heatmap")
knitr::include_graphics(heatmaps[grepl("chevron-circle", heatmaps)])
@

<<corks-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.scap = "Heatmaps of an image containing none of the target classes.", fig.cap = "Heatmaps of an image containing none of the target classes. This outsole shows repeated outlines of corks; the convex corners where the corks overlap are predicted to be stars, and the round edges lead to predictions of circles.", fig.pos = 'b'>>=
knitr::include_graphics(heatmaps[grepl("corks", heatmaps)])
@

<<quad-triangle-hexagon-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.scap = "Heatmaps of an image containing none of the target classes.", fig.cap = "Heatmaps of an image containing quadrilaterals, triangles, and hexagons. The three prominent shapes in this image are all correctly identified, and the heatmap indicates that CoNNOR is using appropriate features for this classification.", fig.pos = 't'>>=
knitr::include_graphics(heatmaps[grepl("quad-triangle-polygon", heatmaps)])
@

<<star-quad-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.scap = "Heatmaps of an image containing quadrilaterals and a star.", fig.cap = "Heatmaps of an image containing quadrilaterals and a star. CoNNOR correctly identifies the quadrilaterals by looking at their corners, and the star is identified by the acute angles of its convex portions.", fig.pos = 'b'>>=
knitr::include_graphics(heatmaps[grepl("star-quad", heatmaps)])
@

<<text-seychelles-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.scap = "Heatmaps of an image containing text.", fig.cap = "Heatmaps of an image containing text. This heatmap clearly shows that the high probability of a circle is a result of the highly curled tails of the 'S' in the text.", fig.pos = 't'>>=
knitr::include_graphics(heatmaps[grepl("text", heatmaps)])
@

<<bowtie-star-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.cap = "Heatmaps of an image containing bowties and a star.", fig.cap = "Heatmaps of an image containing bowties and a star. CoNNOR identifies the star by its pointed ends, but appears to have difficulty locating and identifying the bowties.", fig.pos = 'b'>>=
