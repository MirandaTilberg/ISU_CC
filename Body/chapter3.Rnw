...
\begin{itemize}
\item Example of model prediction \mt{Lots of these now in the model consistency section.}
\item TPR, FPR, and EER
\item Confusion matrix (at EER)
\item ROC/AUC
\end{itemize}

<<roc-code, fig.width = 8, fig.height = 6, out.width = "\\textwidth">>=
library(pROC)
...
<<confusion_matrix, fig.width = 12, fig.height = 23, out.width = ".6\\textwidth", fig.cap = "", fig.scap = "Features detected in a chevron pattern across different colors.", dpi = 600>>=
