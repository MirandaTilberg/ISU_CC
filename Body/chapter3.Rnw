...
The impact of contrast is likely a result of the artificial shoe images being used for classification. VGG16 was trained to recognize images of subjects that are more naturally-occuring and are photographs taken in real settings, such as in \autoref{fig:retrievers}. The outsole images used in this research, on the other hand, are much more artificial images, and any color variation in the image is part of the shoe design. Thus, the "color knowledge", so to speak, of VGG16 is not perfectly generalizable to the domain of classifying outsole images. 
...
In the more homogenous contrast situation shown in \autoref{fig:chevron}, the color variation does not produce large variations in the output probabilities---the darkest image has only slightly lower probability than the lightest images. On the other hand, \autoref{fig:uggs}, \autoref{fig:uggs-with-circle}, \autoref{fig:nb-logo}, and \autoref{fig:nike} show cases where CoNNOR is able to identify shapes when contrast is moderate, but has greater difficulty when contrast is very low. \autoref{ch3:contrast-adjustment} explores the effect of contrast adjustment on CoNNOR's predictions.
