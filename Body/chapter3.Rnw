...
In the figures in this subsection, heatmaps show the three classes with the highest output probabilities alongside the original image. \autoref{fig:chevron-circle-heatmap} shows a case where the chevron and circle predictions are quite good, but there is a line dividing the two sections that is falsely predicted to be a quadrilateral. \autoref{fig:corks-heatmap} shows an outsole with cork shapes; the model bases its high prediction of star on the concave corners where the corks meet. This example also shows the importance of including the category ``other" during model training, as the model must be evaluated on images which do not contain any of the classes. The original image in \autoref{fig:quad-triangle-hexagon-heatmap} contains repeated patterns of quadrilaterals, hexagons, and triangles; CoNNOR predicts all three classes correctly, and the heatmap indicates that the appropriate image regions are being used for these classifications. 

\autoref{fig:star-quad-heatmap} shows correct predictions for both quadrilateral and star, but the false positive label for triangle is not necessarily a fault of the model; the very small center of the star blurs the distinction between a single star and multiple triangles. CoNNOR's confident prediction of a circle in  \autoref{fig:text-seychelles-heatmap} is not technically correct, but the heatmap identifies the highly curved tails of the 'S' as the basis for this prediction, and it is encouraging that strong curvature is closely linked with CoNNOR's understanding of circles. \autoref{fig:bowtie-star-heatmap}, on the other hand, reveals that the classification of bowties is difficult for CoNNOR in certain contexts.
