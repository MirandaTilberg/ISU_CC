...
CoNNOR generally performs well when predicting well-defined shapes. \autoref{fig:generic-chevron}, \autoref{fig:generic-text-line}, and \autoref{fig:generic-triangle} show a number of test images containing chevrons, lines, text, and triangles. Performance varies across these images for a number of reasons. For one, the presence of other shapes can often dominate prediction, such as in \autoref{fig:generic-chevron} where a chevron is made up of quadrilaterals, or in \autoref{fig:generic-triangle} where a logo with triangles also contains text. Prediction is also less accurate for images with low contrast, which can mask the shapes in images beyond the point of recognition by the model.
