...
The following figures show a number of interesting examples, where the heatmaps show the three classes to which CoNNOR assigns the highest probabilities. \autoref{fig:chevron-circle-heatmap} shows a case where the chevron and circle predictions are quite good, but there is a ribbon dividing the two sections that is falsely predicted to be a quadrilateral. \autoref{fig:corks-heatmap} shows an outsole with cork shapes; the model bases its high prediction of star on the concave corners where the corks meet. This example also shows the importance of including the category "other", as the model must be evaluated on images which do not contain any of the classes.

\mt{Very final thing: describe these to take up the whole page}
\autoref{fig:quad-triangle-hexagon-heatmap}
\autoref{fig:star-quad-heatmap}
\autoref{fig:text-seychelles-heatmap}
\autoref{fig:bowtie-star-heatmap}
...
<<bowtie-star-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.cap = "Heatmaps of an image containing bowties and a star.", fig.cap = "CoNNOR identifies the star by its pointed ends, but appears to have difficulty locating and identifying the bowties.", fig.scap = "Heatmaps of an image containing stars and bowties.">>=
