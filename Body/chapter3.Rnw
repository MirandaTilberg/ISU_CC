...
<<generic-text-line, fig.width = 11, fig.height = 6, out.width = "\\textwidth", fig.cap = "A selection of images containing text and/or lines. Most images are correctly identified as containing text, though again there is some indication that low-contrast images are poorly recognized. In addition, text which occurs towards the boundaries of the labeled region may not be successfully identified by the model.", fig.scap = "Features detected in  images with `text' and/or `line' labels.", dpi = 600>>=
...
<<nb_logo, fig.width = 11, fig.height = 11, out.width = "\\textwidth", fig.cap = "New Balance logos found on several different shoes; note that the model can sometimes detect the logo even with color/texture changes, but when the color pattern is of higher contrast than the logo, the model can no longer successfully recognize the logo.", fig.scap = "Features detected in the New Balance shoe logo across different shoe models.", dpi = 600>>=
...
<<corks-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.scap = "Heatmaps of an image containing none of the target classes.", fig.cap = "Heatmaps of an image containing none of the target classes. This outsole shows repeated outlines of corks; the convex corners where the corks overlap are predicted to be stars, and the round edges lead to predictions of circles.", fig.pos = 'h'>>=
...
<<bowtie-star-heatmap, eval = T, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.cap = "Heatmaps of an image containing bowties and a star.", fig.cap = "Heatmaps of an image containing bowties and a star. CoNNOR identifies the star by its pointed ends, but appears to have difficulty locating and identifying the bowties.", fig.pos = '!h'>>=
